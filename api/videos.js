...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    const { data, error } = await supabase
      .from('videos')
      // ✅ genre incluido — antes faltaba y los chips/filtros no lo mostraban
  ...
  if (req.method === 'POST') {
    ...
    // Broadcast a todos los usuarios: nuevo video disponible
  ...
  if (req.method === 'PUT') {
    ...
    const { data, error } = await supabase
      .from('videos')
      .update({ title, drive_url, category, genre, year, description, thumb_url })
